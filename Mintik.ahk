...
Fractions:
    ...
    Gui, Fractions: Add, Tab, x0 y0 h766 w1100, 1|2|3|4
    Gui, Fractions: Color, 1e2124
    Gui, Fractions: Font, s9, Segoe UI
    Gui, Fractions: Font, cwhite
Gui, Fractions: Add, Text, x106 y24  h20 +0x300 cred, БатлПасс
    Gui, Fractions: Add, Text, x8 y40  h20 +0x300,.опыт - Чтобы улучшить боевой пропуск - F2 > Сезонный пропуск > Купить опыт`n-
    Gui, Fractions: Add, Text, x8 y56  h20 +0x300,.дрифт2 - Очки дрифта идут в статистику при набирание 3000 очков за раз и более.`n-
    Gui, Fractions: Add, Text, x8 y72  h20 +0x300,.дрифт - Дрифт счётчик можно включить через F2 > Настройки. Он появляется во время управляемого заноса и показывает, на сколько хорошо вы прошли поворот.На карте вы можете увидеть зоны, где можно дрифтить и попасть в список топ-дрифтеров. Для этого Вам обязательно дрифтить по отмеченной зоне. В противном случае очки не засчитаются.`n-
    Gui, Fractions: Add, Text, x8 y88  h20 +0x300,.байк - Мотоцикл должен быть Ваш, нажимаете ctrl и едете на заднем колесе. `n-
    Gui, Fractions: Add, Text, x8 y104  h20 +0x300,.гв -  Это режим на Maze Bank Arena. Ее можете найти на карте.`n-
    Gui, Fractions: Add, Text, x8 y120  h20 +0x300,.гриль - Краснопёрка - 600грам; Лещ - 600грам; Плотва - 600грам; Вобла - 600грам; Коричневый сом - 750грам; с этих граммовок Вы сможете пожарить 1 рыбу.`n-
    Gui, Fractions: Add, Text, x8 y136  h20 +0x300,.зонадрифта - Чтобы найти зону для дрифта откройте карту. На карте будет изображена в виде колеса в огне. Приезжаете, нажимаете Е на точке (синия метка) и вы в дрифт зоне.`n-
    Gui, Fractions: Add, Text, x8 y152  h20 +0x300,.автопилот -  Чтобы использовать автопилот, Вам необходимо зайти в F2-Настройки-Бинд клавиш, далее поставить удобный для Вас бинд. Выйти с настроек - поставить метку в авто и нажать ваш бинд. `n-
    Gui, Fractions: Add, Text, x8 y168  h20 +0x300,.предметы - Чтобы забрать предметы с рулетки или с сезонного пропуска, Вам необходимо нажать F2 > Магазин > Предметы. `n-
    Gui, Fractions: Add, Text, x8 y184  h20 +0x300,.билет - Розыгрыш проходит 3 раза в день - в 15:00, в 18:00 и в 21:00.`n-
    Gui, Fractions: Add, Text, x8 y200  h20 +0x300,.стриптиз - Чтобы заказать приватный танец, Вам необходимо прийти в клуб. Далее найти стол, подойдя к нему нажать Е. `n-
    Gui, Fractions: Add, Text, x8 y216  h20 +0x300,.мусор - Чтобы открыть мусорку, нужно подойти к ней и нажать Е, однако не все мусорки можно просмотреть. Расположение мусорок Вы можете узнать на сайте https://wiki.majestic-rp.ru/ru/map/rubbish, либо же самостоятельно. `n-
    Gui, Fractions: Add, Text, x8 y232  h20 +0x300, .руки - Чтобы прокатиться несколько метров, Вам необходимо чтобы Вас взял игрок на руки и пронес несколько метров. `n-
    Gui, Fractions: Add, Text, x8 y248  h20 +0x300,.лестн - Вам нужно найти вертикальную лестницу, после подняться на самый верх. Спускаться можете зажав кнопку Shift. Лестница есть у 1 АвтоСервиса.`n-
    Gui, Fractions: Add, Text, x8 y264  h20 +0x300,.тел - Телефоны находятся по всей карте, вот их местоположение на карте: https://wiki.majestic-rp.ru/ru/map/telephone. Чтобы ответить на звонок вам необходимо подойти к нему и нажать Е. Его можно найти по характерному звуку (звонок).`n-
    Gui, Fractions: Add, Text, x8 y280  h20 +0x300,.угонбп - Чтобы угнать транспорт, Вам необходимо находиться в криминальной организации. Далее купить прогромматор ЭБУ на рынке. Сесть на водительское место - G - взломать. 
    Gui, Fractions: Add, Text, x8 y296  h20 +0x300,.животные - Данные животные можно разделать на мясо - кролики, кабаны, олени, коровы, свиньи, петухи. Домашних животных которые гуляют по карте, можно гладить. Животные появляются по всей карте.`n-
    Gui, Fractions: Add, Text, x8 y312  h20 +0x300,.резня - Чтобы разделать животное вам необходимо иметь при себе в руках нож (покупается в магазине 24/7), далее подходите к животному и нажимате G - разделать животное.
    Gui, Fractions: Add, Text, x8 y328  h20 +0x300,.болезнь - Вы не сможете заболеть до 3-го уровня персонажа включительно. Если вас интересует выполнение задания, вы можете заменить его или же повысить уровень персонажа. Более, к сожалению, никак. `n-
    Gui, Fractions: Add, Text, x8 y344  h20 +0x300,.лифт - Для выполнения задания с лифтом в боевом пропуске, нужно воспользоваться такими лифтами, которые установлены в офисе FIB/Weazel News/Мэрии.`n-
    Gui, Fractions: Add, Text, x8 y360  h20 +0x300,.клад - Для поиска кладов Вам необходим металоискатель и лопата, купить можно у NPC на рынке. Металлоискатель становится активным как только Вы берете его в руки. Когда он найдет сокровища, то будут происходить визуальные и звуковые оповещения. От зеленого цвета, до красного. Места расположения сокровищ: пляжи и архипелаги.`n-
    Gui, Fractions: Add, Text, x8 y376  h20 +0x300,.ганг - Maze bank arena (гангейм) - это место, где можно посоревноваться с другими игроками в навыках стрельбы. На карте находится снизу, а изображен в виде джойстика. `n-
    Gui, Fractions: Add, Text, x8 y392  h20 +0x300,.арм - Армрестлинг находится около в Палето около шерифов, а также около №1 Магизна 24/7.`n-
    Gui, Fractions: Add, Text, x8 y408  h20 +0x300,.маг - Можете купить в Магазине 24/7, либо на Маркетплейсе.`n-
    Gui, Fractions: Add, Text, x8 y424  h20 +0x300,.номер - Приветствую, чтобы узнать список бизнесов для ограбления позвоните по следующему номеру - 28121969.`n-
    Gui, Fractions: Add, Text, x8 y440  h20 +0x300,.кд - Перезарядка аренды авто 10 минут.`n-

    Gui, Fractions: Add, Text, x8 y456  h20 +0x300,.рыбмагаз - Все предметы для рыбалки продаются в рыболовном магазине, они убраны из 24/7.Подсаки купить нельзя, можно только крафтить в домашних мастерских.`n-
    Gui, Fractions: Add, Text, x8 y472  h20 +0x300,.бинокль - Для того, чтобы использовать бинокль. Вам нужно открыть инвентарь, после навестись на бинокль в инвентаре, нажать ПКМ и использовать. После зажмите ПКМ и у Вас сработает бинокль.`n-


	Gui, Fractions: Tab, 2
	Gui, Fractions: Add, Text, x106 y24  h20 +0x300 cred, назв
    Gui, Fractions: Add, Text, x8 y40  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y56  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y72  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y88  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y104  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y120  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y136  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y152  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y168  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y184  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y200  h20 +0x300,
    Gui, Fractions: Add, Text, x8 y216  h20 +0x300,
	...
	Gui, Fractions: Tab, 4
    Gui, Fractions: Add, Text, x106 y24  h20 +0x300 cred, Семья
    Gui, Fractions: Add, Text, x8 y40  h20 +0x300, .улучш -  Пока у вас не прокачано в разделе Планшет-Семья-Улучшение определённый контракт/действие, оно не будет доступно в разделе Планшет-Семья-Контракты или же функционал того или иного действия. Пример: нельзя нападать на ФЗ семье, не имея открытого улучшения по нападению на ФЗ.Нельзя открыть контракт Курьер Green, если в Улучшених он не разблокирован.
    Gui, Fractions: Add, Text, x8 y56  h20 +0x300,.тайн -  По всей карте есть тайники Механические, Химические, Техничечские. Тайники есть даже на острове. Чтобы его открыть - необхожимо нажать на Е. Есть определённое КД на спавн предметогв в нём. Если вы будете стоять рядом с ним всё время - содержимое не обновится.
    Gui, Fractions: Add, Text, x8 y72  h20 +0x300, .угон - Угон доступен только для крайм фракций и семей, у которых активно улучшение контрактов для угона авто. Если все условия выполнены, то вот гайд, как начать возить Угнанные машины:https://wiki.majestic-rp.ru/ru/post/ugon-avto.
    Gui, Fractions: Add, Text, x8 y88  h20 +0x300, 
    Gui, Fractions: Add, Text, x8 y104  h20 +0x300, 
    Gui, Fractions: Add, Text, x8 y120  h20 +0x300, 
	Gui, Fractions: Add, Text, x8 y136  h20 +0x300, 
    Gui, Fractions: Show, h414 w300, %Fractions%
	Return
;===================================================================================
...
:?:.техдс::Напишите в технический раздел официального дискорд сервера "Помощь по игре" - "тех-поддержка". Сслылка на канал - https://discord.com/channels/530693013622489088/696711470783922247
...
:?:.угонбп::Чтобы угнать транспорт, Вам необходимо находиться в криминальной организации. Далее купить прогромматор ЭБУ на рынке. Сесть на водительское место - G - взломать. 
