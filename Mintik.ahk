...
Gui, Main: -SysMenu
Gui, Main: Show, w732 h400, Admin.ahk by Mint & dazzziee
...
Gui, Main: Add, Text, x12 y382 w110 h15 +0x200 +BackgroundTrans +border, ahk by Mint & dazzziee
...
;===================================================================================
    ...
    Gui, Answers: Add, Text, x8 y40  h20 +0x200, .багвойс - Перезайдите несколько раз. Методы исправления войс-чата, если множественные перезаходы не помогли: 1. Отключите все аудиовходы и выходы, кроме необходимых (Настройки -> Система -> Звук -> Дополнительные настройки звука -> Правой кнопкой мыши -> Отключить). 2. Удалите аудиоустройство (Диспетчер устройств -> Ввод -> Выберите устройство) и перезагрузите компьютер. 3. Попробуйте изменить IP с помощью различных программ, чтобы подключиться к голосовому серверу.
    Gui, Answers: Add, Text, x8 y56  h20 +0x200, .метал - Металлоискатель активируется при взятии в руки. При нахождении сокровищ происходят визуальные и звуковые оповещения от зеленого до красного цвета. Места расположения сокровищ: пляжи и архипелаги.
    Gui, Answers: Add, Text, x8 y88  h20 +0x200, .рыбалка - Для рыбалки необходимы удочка и наживка, которые можно купить в любом магазине 24/7. Подойдите к зоне рыбалки соответствующего уровня и нажмите клавишу Е. Ждите клёва. Когда появится рыба, она будет вилять в стороны. Зажимайте клавиши A/D противоположной стороне движения рыбы. После того как рыба утомится, подсеките её левой кнопкой мыши.
    Gui, Answers: Add, Text, x8 y120  h20 +0x200, .рыб - Ловите рыбу в зоне, соответствующей вашему уровню. 1 уровень - Зоны: 1, 2, 3, 4. Рыба: Форель. Необходимый опыт для повышения: 196. 2 уровень - Зоны: 5, 6. Рыба: Камбала. Необходимый опыт для повышения: 588. 3 уровень - Зоны: 7, 8. Рыба: Тунец. Необходимый опыт для повышения: 1225. 4 уровень - Зоны: 9, 10. Рыба: Красный солдат. Необходимый опыт для повышения: 2205. 5 уровень - Зона: 11. Рыба: Карась. Необходимый опыт для повышения: 3920. 6 уровень - Зона: 12. Рыба: Оранжевый карп. Необходимый опыт для повышения: 6125. 7 уровень - Зона: 13. Рыба: Аквамарин. Необходимый опыт для повышения: 8820. 8 уровень - Зона: 14. Рыба: Золотая рыбка.
    Gui, Answers: Add, Text, x8 y184  h20 +0x200, .ставка - Расчёт ставки происходит в течение 6 часов. Администрация не управляет процессом расчёта.
    Gui, Answers: Add, Text, x8 y200  h20 +0x200, .клвл - Награды за достижение уровней: 5 уровень - 500 MC, 10 уровень - 1000 MC, 15 уровень - 2000 MC, 20 уровень - 3000 MC, 25 уровень - 4000 MC, 30 уровень - 5000 MC. За каждый уровень после 30-го - 1500 MC.
    Gui, Answers: Add, Text, x8 y232  h20 +0x200, .квест - Доступны Мировые и Личные квесты. Подробнее о них можно узнать на рынке (на карте обозначен "Красный вопрос"). Еженедельно доступны 3 личных и 3 мировых квеста. Квесты могут выполнять несколько игроков одновременно, например, инкассатор, мусорщик или почтальон.
    Gui, Answers: Add, Text, x8 y280  h20 +0x200, .фед - На первом этаже находится NPC, у которого можно взять задание. Чтобы отбыть срок в федеральной тюрьме, нужно выполнять задачи (например, мыть туалет). В определённое время тюрьма закрывает клетки на 5 минут. Время открытия и закрытия: 00 мин - открытие, 20 мин - закрытие на 5 минут, 25 мин - открытие, 55 мин - закрытие на 5 минут.
    Gui, Answers: Add, Text, x8 y328  h20 +0x200, .сейф - Закрывать интерфейс сейфа можно только через ESC. В сейф можно положить до 5 миллионов. Деньги идут из налички. Нельзя продать дом с деньгами в сейфе. Если дом слетает по налогам, деньги возвращаются в наличку.
    Gui, Answers: Add, Text, x8 y360  h20 +0x200, .рация - За фракциями закреплены частоты, которые нельзя занять. В F2 -> Настройки можно изменить громкость рации.
    Gui, Answers: Add, Text, x8 y376  h20 +0x200, .лесоруб - Устроиться можно у NPC около Палето-Бей. Для работы нужен топор (покупается у NPC на рынке). Точки для сруба деревьев разбросаны по карте. Древесину можно продать на рынке.
    Gui, Answers: Add, Text, x8 y424  h20 +0x200, .тайм - Визуальный баг таймера. Коины начислятся по истечении времени.
    Gui, Answers: Add, Text, x8 y440  h20 +0x200, .сид - Укажите static ID нарушителя.
    Gui, Answers: Add, Text, x8 y456  h20 +0x200, .пут - Чтобы найти путеводителя, откройте карту и найдите значок в виде человека, машущего рукой. Подойдите и нажмите Е. Будет доступна аренда авто и начальный квест.
    Gui, Answers: Add, Text, x8 y472  h20 +0x200, .увал - Обратитесь к лидеру или заместителю, либо свяжитесь с куратором своей фракции через Discord.
    Gui, Answers: Add, Text, x8 y488  h20 +0x200, .майки - Сначала купите элемент верхней одежды (например, пиджак). После этого будут доступны все майки, подходящие под неё, в разделе "Майки".
    Gui, Answers: Add, Text, x8 y536  h20 +0x200, .баг - Проблема передана разработчикам. Приносим извинения за неудобства.
    Gui, Answers: Add, Text, x8 y584  h20 +0x200, .необл - Не обладаем данной информацией.
    Gui, Answers: Add, Text, x8 y680  h20 +0x200, .функ - Данный функционал временно недоступен.
	...
	Gui, Answers: Tab, 2
    Gui, Answers: Add, Text, x8 y24  h20 +0x200, .бизограб - Для ограбления бизнеса необходимо надеть маску, приобрести оружие и, прибыв в магазин в составе от 2 до 35 человек, навести огнестрельное оружие на NPC (продавца).
    Gui, Answers: Add, Text, x8 y40  h20 +0x200, .ганлиц - Лицензию на оружие можно получить в LSPD или LSCSD.
    Gui, Answers: Add, Text, x8 y56  h20 +0x200, .канистра - Чтобы использовать канистру, возьмите её в руки, нажмите G на автомобиле и выберите опцию "Заправить".
    Gui, Answers: Add, Text, x8 y72  h20 +0x200, .емс - Администрация не оказывает помощь в поднятии или лечении игроков. Обратитесь за помощью к сотрудникам EMS.
    Gui, Answers: Add, Text, x8 y88  h20 +0x200, .подним - Без полной информации о ситуации администрация не может поднимать, лечить или добивать игроков. Дождитесь прибытия EMS или окончания таймера смерти.
    Gui, Answers: Add, Text, x8 y104  h20 +0x200, .жб - Администрация не выносит наказания и не принимает решения без полного понимания ситуации. При наличии видеодоказательств оформите жалобу на форуме.
    Gui, Answers: Add, Text, x8 y120  h20 +0x200, .анак - Для обжалования наказания обратитесь в Discord к администратору, который его вынес.
    Gui, Answers: Add, Text, x8 y136  h20 +0x200, .адз - Администратор сейчас занят или временно отсутствует за компьютером. Напишите ему личное сообщение в Discord.
    Gui, Answers: Add, Text, x8 y152  h20 +0x200, .адс - Администратор отсутствует на сервере. Свяжитесь с ним через личные сообщения в Discord.
    Gui, Answers: Add, Text, x8 y168  h20 +0x200, .погода - Администрация не контролирует погодные условия. Погода меняется автоматически.
    Gui, Answers: Add, Text, x8 y184  h20 +0x200, .неп - Опишите свою проблему или вопрос более подробно для получения точного ответа.
    Gui, Answers: Add, Text, x8 y200  h20 +0x200, .донат - Для получения информации по донату обратитесь по электронной почте help@majestic-rp.ru.
    Gui, Answers: Add, Text, x8 y216  h20 +0x200, .ремонт - Для ремонта автомобиля купите запчасть на АЗС, откройте капот и нажмите G, затем выберите необходимую опцию ремонта.
    Gui, Answers: Add, Text, x8 y232  h20 +0x200, .ферм - Для посадки различных культур на ферме требуется определённый уровень навыков: Пшеница - 225 опыта, Картофель - 950 опыта, Капуста - 2000 опыта, Кукуруза - 3000 опыта, Тыква - 6200 опыта, Бананы - 10500 опыта.
    Gui, Answers: Add, Text, x8 y264  h20 +0x200, .гос - Вы можете сдать автомобиль на свалку и получить 75 процентов от его государственной стоимости. Местоположение свалки обозначено на карте красным перечёркнутым кругом.
    Gui, Answers: Add, Text, x8 y280  h20 +0x200, .аним - Чтобы остановить анимацию, нажмите X и выберите "Остановить анимацию" или нажмите Backspace.
    Gui, Answers: Add, Text, x8 y296  h20 +0x200, .ид - Укажите ID нарушителя.
    Gui, Answers: Add, Text, x8 y312  h20 +0x200, .сто - Уровень износа деталей автомобиля можно проверить в автомастерской (иконка гаечного ключа с отверткой на карте).
    Gui, Answers: Add, Text, x8 y328  h20 +0x200, .жба - Если вы не согласны с решением администратора, подайте жалобу на форуме.
    Gui, Answers: Add, Text, x8 y344  h20 +0x200, .дон - Донатить можно на официальном сайте majestic-rp.ru/donate.
    Gui, Answers: Add, Text, x8 y360  h20 +0x200, .группа - Для работы в кооперативе создайте группу через приложение "Группа" на телефоне и пригласите игроков.
    Gui, Answers: Add, Text, x8 y376  h20 +0x200, .грибы - Для сбора грибов отправляйтесь на одну из точек "Грибы" и возьмите нож в руки.
    Gui, Answers: Add, Text, x8 y392  h20 +0x200, .гриб - Грибы можно продать на рынке.
    Gui, Answers: Add, Text, x8 y408  h20 +0x200, .рын - Рынки расположены на пляже в Лос-Сантосе возле тренажёров и в Палето-Бэй.
    Gui, Answers: Add, Text, x8 y424  h20 +0x200, .оружрын - Сдать оружие боту на рынке можно только при 100 процентов износе.
    Gui, Answers: Add, Text, x8 y440  h20 +0x200, .новость - Для подачи новости обратитесь лично в Weazel News.
    Gui, Answers: Add, Text, x8 y472  h20 +0x200, .нов - Следите за новостями сервера в официальном Discord проекта.
    Gui, Answers: Add, Text, x8 y488  h20 +0x200, .ново - Новости доступны в официальном Discord проекта.
    Gui, Answers: Add, Text, x8 y504  h20 +0x200, .ник - Для смены ника нажмите F2 > Донат > Настройки персонажа.
    Gui, Answers: Add, Text, x8 y616  h20 +0x200, .дублик - Чтобы создать дубликат ключей от авто, купите заготовку в магазине 24/7, затем нажмите G на авто и выберите "Сделать дубликат".
    Gui, Answers: Add, Text, x8 y664  h20 +0x200, .подсел - Чтобы подселить игрока в дом или квартиру, наведитесь на игрока возле дома и нажмите G, затем выберите "Подселить".
    Gui, Answers: Add, Text, x8 y680  h20 +0x200, .краш - При наличии доказательств краша предоставьте их, и вас выпустят.
    Gui, Answers: Add, Text, x8 y696  h20 +0x200, .замок - Для замены дверного замка приобретите "Дверной замок" и "Набор инструментов" на ближайшей заправке, откройте капот (G) и выберите опцию замены.
    Gui, Answers: Add, Text, x8 y728  h20 +0x200, .масло - Для замены масла приобретите "Моторное масло" и "Набор инструментов" на ближайшей заправке, откройте капот (G) и выберите "Заменить масло".
    ...
    Gui, Answers: Add, Text, x8 y40  h20 +0x200, .неоф - Не оффтопьте в обращениях.
    Gui, Answers: Add, Text, x8 y56  h20 +0x200, .одворк - При устройстве на работу вы получаете комплект одежды, который автоматически снимается при увольнении. Медицинский комплект остаётся до завершения лечения.
    Gui, Answers: Add, Text, x8 y72  h20 +0x200, .зоо - Животные могут спавниться по всей карте — от крыс до оленей. Некоторые можно разделать и получить мясо, которое можно приготовить и употребить в пищу. Домашних животных можно гладить.
    Gui, Answers: Add, Text, x8 y104  h20 +0x200, .мед - За этим медиапартнёром наблюдает PR-ассистент, поэтому я не могу выносить наказания. Вся ответственность ложится на PR-ассистента.
    Gui, Answers: Add, Text, x8 y120  h20 +0x200, .зак - Это регулируется IC-законами. Ознакомиться с ними можно на форуме: Majestic RolePlay | Server 3 — Government.
    Gui, Answers: Add, Text, x8 y136  h20 +0x200, .оск - Жалобы на OOC-оскорбления принимаются только при наличии скриншота, сделанного в момент оскорбления, и с видимым ником нарушителя.
    Gui, Answers: Add, Text, x8 y152  h20 +0x200, .аукц - На аукцион можно выставить авто, бизнес, дом или квартиру. В это время авто можно использовать только для передвижения. При продаже дома кладовка переходит покупателю.
    Gui, Answers: Add, Text, x8 y168  h20 +0x200, .болгарка - Чтобы распилить багажник, подойдите к нему, нажмите G, выберите "Нелегальное для авто" и опцию "Распилить дверной замок". Взломать можно гос- и крайм-транспорт, но не личные авто.
    Gui, Answers: Add, Text, x8 y232  h20 +0x200, .вот - Выборы проходят с 10:00 до 10:00 (МСК). Голосовать могут персонажи от 3-го уровня, по принципу "один аккаунт — один голос", в мэрии (отмечено флагом США).
    Gui, Answers: Add, Text, x8 y264  h20 +0x200, .дрифт - Дрифт-счётчик включается через F2 > Настройки. Для попадания в топ необходимо дрифтить только в отмеченных зонах на карте.
    Gui, Answers: Add, Text, x8 y296  h20 +0x200, .скин - Скин на оружие можно активировать через F2 — Магазин — Скины. После активации скин применяется ко всем оружиям этого типа и привязывается к игроку.
    Gui, Answers: Add, Text, x8 y328  h20 +0x200, .огр - Для ограбления банка позвоните по номеру 28121903. Нужен "План ограблений" и минимум 4 участника. Доступны взлом дверей и ячеек. Подробности — внутри игры.
    Gui, Answers: Add, Text, x8 y456  h20 +0x200, .зкайо - Захват точек с материалами (медицинские, военные, технические) на острове Кайо-Перико. Для активации точки используйте соответствующие маячки.
    Gui, Answers: Add, Text, x8 y536  h20 +0x200, .перико - Телепорты доступны для силовых фракций (кроме EMS/WN/GOV) на Кайо-Перико. SANG имеет доступ к складу аналогичному складу на форте.
    Gui, Answers: Add, Text, x8 y600  h20 +0x200, .сирена - Вышку можно взломать с помощью "Программатора ЭБУ" и "Кабеля радио сигналов" (добывается при ограблении 24/7). После удачного взлома статус захвата точек не обновляется 10 минут.
    Gui, Answers: Add, Text, x8 y648  h20 +0x200, .лобби - Создайте собственное лобби для друзей или игроков. Настройки: режим, карта, пароль, оружие, баланс команд и др. Стоимость — от 1250$ до 2800$.
    Gui, Answers: Add, Text, x8 y712  h20 +0x200, .флаг - Режим "Захват флага". Покупайте оружие за внутриигровую валюту и захватывайте точки для победы. Поднимать оружие можно на "F", покупки доступны через "B".


	Gui, Answers: Tab, 5
    Gui, Answers: Add, Text, x8 y24 h20 0x200, .роз - Добавлена система преследования. Для того чтобы начать преследование, нужно выдать розыск — зайти в Планшет - Фракция - База данных - Начать преследование. На карте появится метка местоположения игрока, которая обновляется раз в 30 секунд. Если игрок, которого преследуют, заходит в другой дименшен/интерьер или выходит из игры, он пропадает с радаров. Если государственный игрок подъезжает к метке/подходит к игроку — преследование заканчивается.
    Gui, Answers: Add, Text, x8 y40 h20 0x200, .ар - После окончания срока аренда авто глушится и появляется диалоговое меню, где можно продлить аренду.
    Gui, Answers: Add, Text, x8 y56 h20 0x200, .комт - Рабочий транспорт можно спавнить только на специально отведённом автосервисе для коммерческого транспорта.
    Gui, Answers: Add, Text, x8 y72 h20 0x200, .гр - Лидер группы может дать код для массового добавления людей в группу. Игроки в радиусе 30 метров смогут присоединиться. Лидер и его помощники могут ставить указывающие метки. Это настраивается в F2 - Настройки - Бинды. Лидер может передать свой статус другому игроку.
    Gui, Answers: Add, Text, x8 y88 h20 0x200, .тег - Возможность тегать людей в чате для удобного РП процесса. Для использования напишите @Динамик. Если игрока тегнуть, он получит уведомление. Тег доступен только для ближайших игроков.
    Gui, Answers: Add, Text, x8 y104 h20 0x200, .вчат - Система визуализации чата над игроком: все сообщения из чата дублируются над ним. В настройках - Дополнительно можно включить эту опцию.
    Gui, Answers: Add, Text, x8 y120 h20 0x200, .марк - Маркетплейс используется для размещения различных товаров на продажу, таких как транспорт, недвижимость, бизнес, банкоматы, билборды. Каждое объявление фиксирует количество просмотров и лайков. Нажав «Начать сделку», откроется модальное окно с необходимой информацией и кнопкой для совершения покупки. График отображает динамику цен и имеет возможность взаимодействия. После покупки предметы отправляются в «Склад хранения». За размещение объявления взимается плата в 1.000$ за час (от 5к).
    Gui, Answers: Add, Text, x8 y136 h20 0x200, .схр - Склад хранения — место для предметов, купленных или занесённых игроками для продажи на маркетплейсе. Предметы можно выгрузить из инвентаря или багажника. Склад находится сбоку здания аукциона и не ограничен по объёму. Предметы могут находиться на складе 30 минут, после чего они уходят на аукцион. Имеется возможность добавить карточку в избранное. После покупки имущество может храниться на складе до 2 дней. При входе в зону маркетплейса склад подсвечивается и появляется метка на карте.
    Gui, Answers: Add, Text, x8 y152 h20 0x200, .пар - Положите парашют в быстрый слот, затем в полете нажмите 1, 2 или 3 (в зависимости от слота), после чего ЛКМ — парашют откроется.
    Gui, Answers: Add, Text, x8 y168 h20 0x200, .дмк - Дефибриллятор МК2 не имеет функционала и является аналогом обычного дефибриллятора.
    Gui, Answers: Add, Text, x8 y184 h20 0x200, .адр - Адреналин повышает скорость восстановления в 2 раза быстрее, чем эпинефрин.
    Gui, Answers: Add, Text, x8 y200 h20 0x200, .нагр - Автомобили из пропусков являются эксклюзивами, их нельзя передавать другим игрокам.
    Gui, Answers: Add, Text, x8 y216 h20 0x200, .жап - Предмет "Желтая аптечка" восстанавливает 75 хп.
    Gui, Answers: Add, Text, x8 y232 h20 0x200, .обв - Обвесы доступны в призах. Первая установка бесплатная. Обвес можно снять/поставить отдельно, он закрепляется за машиной.
    Gui, Answers: Add, Text, x8 y248 h20 0x200, .куп - Купоны действуют на все автомобили из Majestic Motors.
    Gui, Answers: Add, Text, x8 y264 h20 0x200, .случ - В наградах есть "Случайные наборы". При получении приза игрок выбирает одну из трёх карт с разным содержимым.
    Gui, Answers: Add, Text, x8 y280 h20 0x200, .оос - ООС зону можно включить в F2 - Настройки - Бинды. При нажатии на клавишу, если игрок перед вами вышел, будет отображена информация о времени и дате его выхода.
    Gui, Answers: Add, Text, x8 y296 h20 0x200, .квр - У бота на работе есть кнопка "Я могу чем-то помочь". При нажатии откроется квест, который нужно выполнить (на F6 можно посмотреть).
    Gui, Answers: Add, Text, x8 y312 h20 0x200, .вбп - Бронзовый пропуск открывает доступ к премиум-наградам до 35 уровня, серебряный — до 75 уровня, золотой — до 110 уровня, платиновый — до 150 уровня. Опыт даёт только королевский старт.
    Gui, Answers: Add, Text, x8 y328 h20 0x200, .алис - Чтобы выключить GPS ассистента, нажмите F2 - Настройки и отключите GPS.
    Gui, Answers: Add, Text, x8 y344 h20 0x200, .закр - Закройте обращение и откройте снова.
    Gui, Answers: Add, Text, x8 y360 h20 0x200, .зп - Для начисления PayDay необходимо иметь банковскую карту и быть уволенным со всех работ. Карта оформляется в любом банке.
    Gui, Answers: Add, Text, x8 y376 h20 0x200, .вод - Администрация не извлекает автомобили из воды, а только удаляет их, чтобы вы могли заспавнить новые. Нужно самостоятельно добраться до автосервиса или нужного места.
    Gui, Answers: Add, Text, x8 y392 h20 0x200, .бенз - Администрация не заправляет автомобили игроков. Вы можете вызвать такси, чтобы добраться до нужного места, или приобрести канистру для заправки на АЗС.
    Gui, Answers: Add, Text, x8 y408 h20 0x200, .удал - Администрация не удаляет автомобили игроков, за исключением случаев, когда они утонули и не исчезают.
    Gui, Answers: Add, Text, x8 y424 h20 0x200, .стат - Статистика доступна в F2 - Персонаж - Статистика.
    Gui, Answers: Add, Text, x8 y440 h20 0x200, .пред - Для выключения музыки используйте команду /asms, если вы находитесь в зеленой зоне.
    Gui, Answers: Add, Text, x8 y456 h20 0x200, .кв - Текущий квест отображается над таймером Majestic Coins. Если его нет — нажмите F6.
    Gui, Answers: Add, Text, x8 y472 h20 0x200, .кмб - Чтобы устроиться в силовую гос. структуру, нужно получить военный билет. Для этого надо устроиться в SANG и пройти КМБ. Иконка зелёного самолёта на карте.
    Gui, Answers: Add, Text, x8 y488 h20 0x200, .нарк - Пакетики Blue и White имеют такие же эффекты, как Green, только с другими эффектами.
    Gui, Answers: Add, Text, x8 y504 h20 0x200, .верт - Вертолёты продаются в салоне недалеко от аэропорта.
    Gui, Answers: Add, Text, x8 y520 h20 0x200, .поход - Для изменения походки и эмоций лица используйте F2 - Настройки - Главное.
    Gui, Answers: Add, Text, x8 y536 h20 0x200, .багаж - Чтобы выйти из багажника, нажмите "E".
    Gui, Answers: Add, Text, x8 y552 h20 0x200, .нал - Для оплаты налогов на дом откройте приложение "Мой дом" в планшете.
...
Fractions:
    ...
    Gui, Fractions: Add, Text, x8 y56  h20 +0x240, .дрифт2 - Очки дрифта идут в статистику при набирание 3000 очков за раз и более.`n-
...
:?:.пок::/asms Удачной Вам игры, и с наступающим Новым Годом. {left 48}
:?:.пока:: Удачной Вам игры, и с наступающим Новым Годом.
:?:/gjr::/asms Удачной Вам игры, и с наступающим Новым Годом. {left 48}
:?:/gjrf:: Удачной Вам игры, и с наступающим Новым Годом.
...
:?:.дрифт2::Очки дрифта идут в статистику при набирание 3000 очков за раз и более.
:?:.дрифт::Дрифт счётчик можно включить через F2 > Настройки. Он появляется во время управляемого заноса и показывает, на сколько хорошо вы прошли поворот.На карте вы можете увидеть зоны, где можно дрифтить и попасть в список топ-дрифтеров. Для этого Вам обязательно дрифтить по отмеченной зоне. В противном случае очки не засчитаются.
...
:?:.рыба::Присылаю Вам поробный Гайд по рыбалке, где есть ответы на все вопросы - https://wiki.majestic-rp.ru/post/rybak
...
:?:.актуал::Обращение неактивно более одной минуты, вынужден его закрыть. Если же у Вас есть не решенный вопрос/проблема создайте новое и ожидайте ответа.
:?:/frnefk::Обращение неактивно более одной минуты, вынужден его закрыть. Если же у Вас есть не решенный вопрос/проблема создайте новое и ожидайте ответа.
...
:?:.зкфтпу::/prange
:?:.зштп::/ping
:?:/x[::Если у вас есть откат с подозрительным моментом, то вы можете предоставить его администрации в с ролью "Cheats Hunter". Найти можно в оф. дискорде проекта.
...
:?:.чх::Если у вас есть откат с подозрительным моментом, то вы можете предоставить его администрации в с ролью "Cheats Hunter". Найти можно в оф. дискорде проекта.
...
:?:.кгз::/ajail 10 Crime in GZ{Left 14}
...
:?:/rup::/ajail 10 Crime in GZ{Left 14}
:?:/119::/ajail 35 1.19 Правила государственных организаций{Left 44}
:?:/1241::/ajail 10 1.24.1 Правила государственных организаций{Left 46}
:?:/111::/ajail 50 1.11 Правила государственных организаций{Left 44}
:?:/122::/ban 10 1.22 Правила государственных организаций{Left 44}
...
;===================================================================================
change:
    MsgBox, 8192, Обновление от 19.12.2024 включает в себя:,
.игр - Игрок получил наказание.`n- .игр2 - Игроки получили наказания.`n- .при - Приятной игры на Majestic RP.`n-.кгз - /ajail  10 Crime in GZ.`n-.оцоск - /ajail  15 OOC osk `n-.амн /ajail  Амнистия`n- Добавлено 1.22, 1.11 ПГО`n- Добавлено .оос - /mute  90 OOC in IC`n- Исправлены ошибки в тексте.`n-.суд Добавлен`n- .пох - Похищение`n- .софт - /hardban  9999 Cheats`n-.лестн - Вам нужно найти вертикальную лестницу, после подняться на самый верх. Спускаться можете зажав кнопку Shift. Лестница есть у 1 АвтоСервиса.`n- Починили уборку снега`n- .раб - Для работы подходят такие работы как: Дальнобойщик, Мусоровозщик, Инкассатор, Шахтер, Почтальон.`n- .маг - Можете купить в Магазине 24/7, либо на Маркетплейсе. `n `n Обновление от 19.12.2024 21:00 включает в себя:`n.цех-> Функциональное мероприятие криминальных и государственных организаций. Проводится 2 раза в день с перерывом в 8 часов. В 14:45 и 22:45. Целью данного мероприятия являеться захват контрольных точек "цехов". Мероприятие проходит в гетто.`n.дил->Дилеры-это мероприятия для криминальных и государственных организаций. Появляются они 2 раза в день с перерывом в 8 часов. В 10:45 и 18:45. Целью мероприятия является вербовка NPC на свою сторону, путем направления оружия. Мероприятие проходит в гетто.`n.кам-> Камеры разбросаны по всей карте. У Гос.сотрудников, есть вкладка в планшете, с использованием камер, с помощью ее они могут выдавать  штрафы за превышение скорости. Для того, чтобы снять камеру, вам нужно состоять в криминальной организации или семье, иметь при себе кусачки и отвертку.`n.рыба - Присылаю Вам поробный Гайд по рыбалке, где есть ответы на все вопросы - https://wiki.majestic-rp.ru/post/rybak
;===================================================================================
