...
;===================================================================================
    ...
    Gui, Answers: Add, Text, x8 y408  h20 +0x200, .рац - Для использования: Возьмите рацию > Введите чистоту >Нажмите ентер >Зажмите "M" и будет слышно.
	...
	Gui, Answers: Add, Text, x8 y536  h20 +0x200, .баг - Знаем о данной проблеме, она уже передана разработчикам. Приносим свои извинения за доставленные неудобства. 
    ...
    Gui, Answers: Add, Text, x8 y712  h20 +0x200, .изв - Приносим свои извинения за доставленные неудобства.
    ...
    Gui, Answers: Add, Text, x8 y360  h20 +0x200, .группа - Чтобы вместе работать на кооперативной работе вам необходимо пригласить игроков через приложение "Группа" в телефоне.
	...
	Gui, Answers: Add, Text, x8 y536  h20 +0x200, месячного лимита комиссия на вывод 5ц проц (Максимум 20.000$), на перевод 4 проц (Максимум 20.000$), кешбэк в магазинах отсутствует. Premium карта стоит в обслуживании 25.000$ в месяц, снятия 
    Gui, Answers: Add, Text, x8 y552  h20 +0x200, и переводы без комиссии до 2.500.000$ после превышения месячного лимита комиссия на вывод 4 проц (Максимум 15.000$), на перевод 3 проц (Максимум 15.000$), кешбэк в магазинах 1 проц, лимит 
    Gui, Answers: Add, Text, x8 y568  h20 +0x200, кешбэка на одну операцию до 2.500$, месячный лимит кешбэка 50.000$. VIP карта стоит в обслуживании 75.000$ в месяц, снятия и переводы без комиссии до 20.000.000$ после превышения месячного 
    Gui, Answers: Add, Text, x8 y584  h20 +0x200, лимита комиссия на вывод 3 проц (Максимум 20.000$), на перевод 2.5 проц (Максимум 20.000$), кешбэк в магазинах 3 проц, лимит кешбэка на одну операцию до 10.000$, месячный лимит кешбэка 
    ...
    Gui, Answers: Add, Text, x8 y216  h20 +0x200,  Количество символов неограниченно, вы можете полностью расписать Вашу проблему/вопрос.
    ...
    Gui, Answers: Add, Text, x8 y200  h20 0x200, еще раз вся необходимая информация, сумма к оплате и кнопка совершить покупку. График, который отображает  динамику цен и имеет возможность взаимодействия. После покупки предм. 
    ...
    Gui, Answers: Add, Text, x8 y264  h20 0x200, можно выгрузить предметы из инвентаря или из багажника. Склад хранения находится сбоку здания аукциона. Склад не ограничен по хранению предметов Предметы могут находиться на складе 30 
    Gui, Answers: Add, Text, x8 y280  h20 0x200, минут, после истечения этого времени они идут на аукцион. Имеется возможность добавить карточку в избранное. После покупки имущество/предметы могут храниться на складе 2 дня. При входе 
    ...
    Gui, Answers: Add, Text, x8 y360  h20 0x200, .нагр - Автомобили из пропуска являются эксклюзивом, их нельзя передавать другим игрокам.
    ...
    Gui, Answers: Add, Text, x8 y472  h20 0x200, .вбп - Бронзовый пропуск открывает доступ к премиум наградам до 35 уровня, серебряный до 75 уровня, золотой до 110 уровня, платиновый до 150 уровня. Опыт дает только королевский старт.
...
;===================================================================================
    ...
    Gui, SkillInfo: Add, Text, x8 y200  h20 +0x240, транспортным средством.
    ...
    Gui, SkillInfo: Add, Text, x8 y248  h20 +0x240, повысить пройдя курсы в летной школе. Пройти их можно 1 раз в 24 часа, увеличение навыка от 1 занятия - 
...
Fractions:
    ...
    Gui, Fractions: Add, Text, x8 y312  h20 +0x240,.резня - Чтобы разделать животное, Вам необходимо иметь при себе в руках нож (покупается в магазине 24/7), далее подходите к животному и нажимаете G - разделать животное.
    ...
    Gui, Fractions: Add, Text, x8 y392  h20 +0x240,.арм - Армреслинг находится около в Палето около шерифов, а также около №1 Магизна 24/7.`n-
...
:?:.рац::Для использования: Возьмите рацию > Введите чистоту >Нажмите ентер >Зажмите "M" и будет слышно.
...
:?:.штраф::Чтобы оплатить штраф нужно приехать на Штраф Стоянку и спуститься вниз. Там будет бот, у которого можно будет оплатить.
...
:?:.вожден::Вождение - навык вождения повышается от времени, проведенного за рулем автомобиля. Чем больше ты водишь транспорт, тем быстрее повышается навык. От прокачки увеличивается управляемость транспортным средством.
...
:?:.ставка::Расчёт ставки обычно происходит в течение 6-ти часов. Администрация не управляет расчетом.
...
:?:.группа::Чтобы вместе работать на кооперативной работе вам необходимо пригласить игроков через приложение "Группа" в телефоне.
...
:?:.выб::Выборы активны с 10:00 - 10:00 по московскому времени. Проголосовать может каждый от 3-го уровня игрового персонажа, по принцыпу один голос на один аккаунт. голосование проходит в левом крыле Мэрии(На карте флаг USA).
...
:?:.цех::Функциональное мероприятие криминальных и государственных организаций. Проводится 2 раза в день с перерывом в 8 часов. В 14:45 и 22:45. Целью данного мероприятия является захват контрольных точек "цехов". Мероприятие проходит в гетто.
...
:?:.нагр::Все автомобили и одежда из пропуска являются эксклюзивом, их нельзя передавать другим игрокам.
...
:?:.вбп::Бронзовый пропуск открывает доступ к премиум наградам до 35 уровня, серебряный до 75 уровня, золотой до 110 уровня, платиновый до 150 уровня. Опыт дает только королевский старт.
...
:?:.работа::Для выполнения "КВЕСТА" на работе вам необходимо подойти к нпс после чего нажать Е. Подобные квесты можно взять на любой из работ (все зеленые чемоданы на карте).
...
:?:.баг::Знаем о данной проблеме, она уже передана разработчикам. Приносим свои извинения за доставленные неудобства. 
...
:?:.реп::Пожалуйста, уточните свой вопрос подробнее. Администрация не летает на репорты по типу "админ тп", "админ можно поговорить", "помогите", "админ есть вопрос". Количество символов неограниченно, вы можете полностью расписать Вашу проблему/вопрос.
...
:?:.исч::Все транспортные средства исчезают через время, если с ними никак не взаимодействовать.
...
:?:.зак::Это регламентируется IC законами, изучить их можете в разделе Government на форуме: https://forum.majestic-rp.ru/ 
...
:?:.пузо::Чтобы "Лечь на Живот". Вам нужно открыть F2 - Позы - Лечь на живот.
...
:?:.дс::Предоставьте видеодоказательства мне в личные сообщения в дискорде:
:?:.погода::К сожалению, администрация не контролирует данный процесс. Погода меняется автоматически.
...
;===================================================================================
change:
    MsgBox, 8192, 01.02.2025 Обновления:`n , .запретка - /ban 3 3.5 Основных Правил Проекта`n
.нар - Рядом находятся другие игроки, которые могут вам помочь. В таких случаях мы не снимаем наручники/стяжки.`n
.крем - Крем-маска - уникальный предмет из Battle Pass. На время действия эффекта не позволяет сверить фоторобот и увидеть ваш никнейм над головой тем, кто поздоровался с вами. Заменяет собой функционал маски. Срок действия - 30 минут.`n
.сигналка - Автосигнализация - уникальный предмет из Battle Pass. Устанавливается на ваш личный транспорт. После того, как вашим авто завладеет тот, у кого нет к нему доступа, вам на телефон придет сообщение об этом.`n
.кастет - Стальной кастет - уникальный предмет из Battle Pass. Используется как оружие. При использовании удар вызывает контузию у вашего противника.`n
.горн - Воздушный горн - уникальный предмет из Battle Pass. Используется из инвентаря и имеет проценты использования. Использование вызывает эффект "глухота" у всех вокруг. У людей совсем близко также проиграется специальная анимация оглушения.`n
.дронскан - Дрон Сканер - уникальный предмет из Battle Pass. Имеет функционал обычного гражданского дрона (без тепловизора) но с дополнительной возможностью отмечать всех людей вокруг на миникарте у вас и вашей группы.`n
.замокшок - Набор дверных шокеров - уникальный предмет из Battle Pass. Устанавливается на ваш личный автомобиль. При попытки сесть в ваш закрытый автомобиль игрок без доступа получит урон и анимацию удара током.`n
return
;===================================================================================
